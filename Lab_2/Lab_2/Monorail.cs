...
namespace WindowsFormsMonorail
{
    ...
    {
        public Color headColor { private set; get; }
        public Color SideColor { private set; get; }
        public Monorail(int maxSpeed, float weight, Color headcolor, Color sidecolor,Color bodycolor, bool window, bool doors) : base(maxSpeed, weight,sidecolor , bodycolor, window, doors)
        {
            MaxSpeed = maxSpeed;
            Weight = weight;
            Window = window;
        ...
        public override void DrawMonorail(Graphics g)
        {
            ...
            float resize = 1.1f;
                ...
                g.FillRectangle(wire, _startPosX - (400 / resize), _startPosY + (50 / resize), 600 / resize, 20 / resize);

          

            Brush monorail = new SolidBrush(SideColor);

            Brush clip = new SolidBrush(Color.RoyalBlue);

            Brush window = new SolidBrush(BodyColor);

            Brush strip = new SolidBrush(headColor);
